...
LAG=$(/opt/kafka/bin/kafka-run-class.sh kafka.tools.ConsumerOffsetChecker --group "${GROUP}" --zkconnect "${ZK}" --topic "${TOPIC}" |grep "${GROUP}" |gawk '{sum+=$6} END {print sum}')

if (( LAG >= ${CRITICAL} ))
then
    echo "CRITICAL|Lag=${LAG}"
    exit 2
else
    if (( LAG >= ${WARNING} ))
    then
        echo "WARNING|Lag=${LAG}"
        exit 1
    fi
    echo "OK|Lag=${LAG}"
    exit 0
